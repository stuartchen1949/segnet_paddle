...
class MaskToTensor(object):
    ...
    def __call__(self, img):
        return paddle.to_tensor(np.array(img, dtype=np.int32)).long()
...
class RandomCrop(object):
    ...
    def __call__(self, img, mask):
        ...
        if w == tw and h == th:
            return img, mask
        if w < tw or h < th:
            return img.resize((tw, th), Image.BILINEAR), mask.resize((tw,
                th), Image.NEAREST)
        x1 = random.randint(0, w - tw)
        y1 = random.randint(0, h - th)
        return img.crop((x1, y1, x1 + tw, y1 + th)), mask.crop((x1, y1, x1 +
            tw, y1 + th))
...
class RandomHorizontallyFlip(object):
    ...
    def __call__(self, img, mask):
        if random.random() < 0.5:
            return img.transpose(Image.FLIP_LEFT_RIGHT), mask.transpose(Image
                .FLIP_LEFT_RIGHT)
        return img, mask
...
class Scale(object):
    ...
    def __call__(self, img, mask):
        ...
        if w > h:
            ...
            return img.resize((ow, oh), Image.BILINEAR), mask.resize((ow,
                oh), Image.NEAREST)
        else:
            oh = self.size
...
class SlidingCropOld(object):
    ...
    def __call__(self, img, mask):
        ...
        if long_size > self.crop_size:
            ...
            h_step_num = int(math.ceil((h - self.crop_size) / float(stride))
                ) + 1
            w_step_num = int(math.ceil((w - self.crop_size) / float(stride))
                ) + 1
            ...
            for yy in range(h_step_num):
                for xx in range(w_step_num):
                    ...
                    img_sublist.append(Image.fromarray(img_sub.astype(np.
                        uint8)).convert('RGB'))
                    mask_sublist.append(Image.fromarray(mask_sub.astype(np.
                        uint8)).convert('P'))
            ...
            for yy in range(h_step_num):
                for xx in range(w_step_num):
                    ...
                    img_sub, mask_sub, sub_h, sub_w = self._pad(img_sub,
                        mask_sub)
                    img_slices.append(Image.fromarray(img_sub.astype(np.
                        uint8)).convert('RGB'))
                    mask_slices.append(Image.fromarray(mask_sub.astype(np.
                        uint8)).convert('P'))
