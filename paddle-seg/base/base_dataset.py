...
class BaseDataSet(Dataset):
    ...
    def __init__(self, root, split, mean, std, base_size=None, augment=True,
        val=False, crop_size=321, scale=True, flip=True, rotate=False, blur
        =False, return_id=False):
        self.root = root
        self.split = split
        self.mean = mean
        self.std = std
        ...
        self.files = []
        self._set_files()
        self.to_tensor = torch2paddle.ToTensor()
        self.normalize = torch2paddle.Normalize(mean, std)
    ...
    def _val_augmentation(self, image, label):
        if self.crop_size:
            ...
            label = Image.fromarray(label).resize((w, h), resample=Image.
                NEAREST)
    ...
    def _augmentation(self, image, label):
        ...
        if self.base_size:
            if self.scale:
                longside = random.randint(int(self.base_size * 0.5), int(
                    self.base_size * 2.0))
            else:
        ...
        if self.rotate:
            ...
            image = cv2.warpAffine(image, rot_matrix, (w, h), flags=cv2.
                INTER_LINEAR)
            label = cv2.warpAffine(label, rot_matrix, (w, h), flags=cv2.
                INTER_NEAREST)
        if self.crop_size:
            pad_h = max(self.crop_size - h, 0)
            pad_w = max(self.crop_size - w, 0)
            pad_kwargs = {'top': 0, 'bottom': pad_h, 'left': 0, 'right':
                pad_w, 'borderType': cv2.BORDER_CONSTANT}
    ...
    def __getitem__(self, index):
        ...
        label = paddle.to_tensor(np.array(label, dtype=np.int32)).long()
