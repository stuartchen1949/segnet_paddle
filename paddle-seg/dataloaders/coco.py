...
class CocoStuff10k(BaseDataSet):
    ...
    def _set_files(self):
        if self.split in ['train', 'test', 'all']:
            file_list = os.path.join(self.root, 'imageLists', self.split +
                '.txt')
            self.files = [name.rstrip() for name in tuple(open(file_list, 'r'))
                ]
        else:
            raise ValueError(
                f'Invalid split name {self.split} choose one of [train, test, all]'
                )
    ...
    def _load_data(self, index):
        ...
        label[label == -1] = 255
        if self.warp_image:
            image = cv2.resize(image, (513, 513), interpolation=cv2.
                INTER_LINEAR)
            label = np.asarray(Image.fromarray(label).resize((513, 513),
...
class CocoStuff164k(BaseDataSet):
    ...
    def _set_files(self):
        if self.split in ['train2017', 'val2017']:
            file_list = sorted(glob(os.path.join(self.root, 'images', self.
                split + '/*.jpg')))
            self.files = [os.path.basename(f).split('.')[0] for f in file_list]
        else:
            raise ValueError(
                f'Invalid split name {self.split}, either train2017 or val2017'
                )

    def _load_data(self, index):
        image_id = self.files[index]
        image_path = os.path.join(self.root, 'images', self.split, image_id +
            '.jpg')
...
class COCO(BaseDataLoader):
    ...
    def __init__(self, data_dir, batch_size, split, crop_size=None,
        base_size=None, scale=True, num_workers=1, partition=\
        'CocoStuff164k', shuffle=False, flip=False, rotate=False, blur=\
        False, augment=False, val_split=None, return_id=False, val=False):
        ...
        if partition == 'CocoStuff10k':
            ...
            raise ValueError(
                'Please choose either CocoStuff10k / CocoStuff164k')
        super(COCO, self).__init__(self.dataset, batch_size, shuffle,
