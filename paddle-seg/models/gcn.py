...
class Block_Resnet_GCN(nn.Layer):

    def __init__(self, kernel_size, in_channels, out_channels, stride=1):
        super(Block_Resnet_GCN, self).__init__()
        self.conv11 = nn.Conv2D(in_channels, out_channels, stride=stride,
            kernel_size=(kernel_size, 1), padding=(kernel_size // 2, 0),
            bias_attr=False)
        self.bn11 = nn.BatchNorm2D(out_channels)
        self.relu11 = x2paddle.torch2paddle.ReLU(inplace=True)
        self.conv12 = nn.Conv2D(out_channels, out_channels, stride=stride,
            kernel_size=(1, kernel_size), padding=(0, kernel_size // 2),
            bias_attr=False)
        self.bn12 = nn.BatchNorm2D(out_channels)
        self.relu12 = x2paddle.torch2paddle.ReLU(inplace=True)
        self.conv21 = nn.Conv2D(in_channels, out_channels, stride=stride,
            kernel_size=(1, kernel_size), padding=(0, kernel_size // 2),
            bias_attr=False)
        self.bn21 = nn.BatchNorm2D(out_channels)
        self.relu21 = x2paddle.torch2paddle.ReLU(inplace=True)
        self.conv22 = nn.Conv2D(out_channels, out_channels, stride=stride,
            kernel_size=(kernel_size, 1), padding=(kernel_size // 2, 0),
            bias_attr=False)
        self.bn22 = nn.BatchNorm2D(out_channels)
        self.relu22 = x2paddle.torch2paddle.ReLU(inplace=True)
...
class BottleneckGCN(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.conv1x1 = nn.Conv2D(out_channels_gcn, out_channels, 1,
            bias_attr=False)
...
class BR_Block(nn.Layer):

    def __init__(self, num_channels):
        super(BR_Block, self).__init__()
        self.bn1 = nn.BatchNorm2D(num_channels)
        self.relu1 = x2paddle.torch2paddle.ReLU(inplace=True)
        self.conv1 = nn.Conv2D(num_channels, num_channels, 3, padding=1)
        self.bn2 = nn.BatchNorm2D(num_channels)
        self.relu2 = x2paddle.torch2paddle.ReLU(inplace=True)
...
class GCN(BaseModel):
    ...
    def __init__(self, num_classes, in_channels=3, pretrained=True,
        use_resnet_gcn=False, backbone='resnet50', use_deconv=False,
        ...
        if self.use_deconv:
            self.decon1 = torch2paddle.Conv2DTranspose(num_classes,
                num_classes, kernel_size=3, padding=1, output_padding=1,
                stride=2, bias=False)
            self.decon2 = torch2paddle.Conv2DTranspose(num_classes,
                num_classes, kernel_size=3, padding=1, output_padding=1,
                stride=2, bias=False)
            self.decon3 = torch2paddle.Conv2DTranspose(num_classes,
                num_classes, kernel_size=3, padding=1, output_padding=1,
                stride=2, bias=False)
            self.decon4 = torch2paddle.Conv2DTranspose(num_classes,
                num_classes, kernel_size=3, padding=1, output_padding=1,
                stride=2, bias=False)
            self.decon5 = torch2paddle.Conv2DTranspose(num_classes,
                num_classes, kernel_size=3, padding=1, output_padding=1,
                stride=2, bias=False)
    ...
    def forward(self, x):
        ...
        if self.use_deconv:
            ...
            x4 = F.interpolate(x4, size=x3.size()[2:], mode='bilinear',
                align_corners=True)
            x3 = F.interpolate(self.br5(x3 + x4), size=x2.size()[2:], mode=\
                'bilinear', align_corners=True)
            x2 = F.interpolate(self.br6(x2 + x3), size=x1.size()[2:], mode=\
                'bilinear', align_corners=True)
            x1 = F.interpolate(self.br7(x1 + x2), size=conv1_sz, mode=\
                'bilinear', align_corners=True)
            x = self.br9(F.interpolate(self.br8(x1), size=x.size()[2:],
