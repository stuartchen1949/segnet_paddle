...
model_urls = {'resnet18':
    ...
    'resnet50':
    'https://s3.us-west-1.wasabisys.com/encoding/models/resnet50s-a75c83cf.zip'
    , 'resnet101':
    'https://s3.us-west-1.wasabisys.com/encoding/models/resnet101s-03a0f310.zip'
    , 'resnet152':
    'https://s3.us-west-1.wasabisys.com/encoding/models/resnet152s-36670e8b.zip'
    }
...
class BasicBlock(nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=\
        ...
        self.conv1 = nn.Conv2D(inplanes, planes, kernel_size=3, stride=\
            stride, padding=dilation, dilation=dilation, bias_attr=False)
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True,
        ...
        if deep_base:
            self.conv1 = nn.Sequential(nn.Conv2D(3, 64, kernel_size=3,
                stride=2, padding=1, bias_attr=False), norm_layer(64),
                x2paddle.torch2paddle.ReLU(inplace=True), nn.Conv2D(64, 64,
                kernel_size=3, stride=1, padding=1, bias_attr=False),
                norm_layer(64), x2paddle.torch2paddle.ReLU(inplace=True),
...
def load_url(url, model_dir='./pretrained', map_location=None):
    ...
    cached_file = os.path.join(model_dir, filename + '.pdiparams')
