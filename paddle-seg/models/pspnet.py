...
class _PSPModule(nn.Layer):
    ...
    def __init__(self, in_channels, bin_sizes, norm_layer):
        ...
        self.stages = nn.LayerList([self._make_stages(in_channels,
        ...
        self.bottleneck = nn.Sequential(nn.Conv2D(in_channels + 
            ...
            padding=1, bias_attr=False), norm_layer(out_channels), x2paddle
            .torch2paddle.ReLU(inplace=True), nn.Dropout(0.1))
    ...
    def _make_stages(self, in_channels, out_channels, bin_sz, norm_layer):
        ...
        relu = x2paddle.torch2paddle.ReLU(inplace=True)
    ...
    def forward(self, features):
        ...
        output = self.bottleneck(torch2paddle.concat(pyramids, dim=1))
        return output


class PSPNet(BaseModel):

    def __init__(self, num_classes, in_channels=3, backbone='resnet152',
        ...
        self.master_branch = nn.Sequential(_PSPModule(m_out_sz, bin_sizes=[
            1, 2, 3, 6], norm_layer=norm_layer), nn.Conv2D(m_out_sz // 4,
            num_classes, kernel_size=1))
        self.auxiliary_branch = nn.Sequential(nn.Conv2D(m_out_sz // 2, 
            m_out_sz // 4, kernel_size=3, padding=1, bias_attr=False),
            norm_layer(m_out_sz // 4), x2paddle.torch2paddle.ReLU(inplace=\
            True), nn.Dropout(0.1), nn.Conv2D(m_out_sz // 4, num_classes,
            kernel_size=1))
    ...
    def get_decoder_params(self):
        return chain(self.master_branch.parameters(), self.auxiliary_branch
            .parameters())
...
class PSPDenseNet(BaseModel):
    ...
    def __init__(self, num_classes, in_channels=3, backbone='densenet201',
        pretrained=True, use_aux=True, freeze_bn=False, **_):
        super(PSPDenseNet, self).__init__()
        self.use_aux = use_aux
        model = getattr(x2paddle.models, backbone)(pretrained)
        m_out_sz = model.classifier.in_features
        aux_out_sz = model.features.transition3.conv.out_channels
        if not pretrained or in_channels != 3:
            block0 = [nn.Conv2D(in_channels, 64, 3, stride=2, bias_attr=\
                False), nn.BatchNorm2D(64), x2paddle.torch2paddle.ReLU(
                inplace=True)]
            block0.extend([nn.Conv2D(64, 64, 3, bias_attr=False), nn.
                BatchNorm2D(64), x2paddle.torch2paddle.ReLU(inplace=True)] * 2)
            self.block0 = nn.Sequential(*block0, nn.MaxPool2D(kernel_size=3,
                stride=2, padding=1))
        ...
        self.transition2 = nn.Sequential(*list(model.features.transition2.
            children())[:-1])
        self.transition3 = nn.Sequential(*list(model.features.transition3.
            children())[:-1])
        ...
        self.master_branch = nn.Sequential(_PSPModule(m_out_sz, bin_sizes=[
            1, 2, 3, 6], norm_layer=nn.BatchNorm2d), nn.Conv2D(m_out_sz // 
            4, num_classes, kernel_size=1))
        self.auxiliary_branch = nn.Sequential(nn.Conv2D(aux_out_sz, 
            m_out_sz // 4, kernel_size=3, padding=1, bias_attr=False), nn.
            BatchNorm2D(m_out_sz // 4), x2paddle.torch2paddle.ReLU(inplace=\
            True), nn.Dropout(0.1), nn.Conv2D(m_out_sz // 4, num_classes,
            kernel_size=1))
        initialize_weights(self.master_branch, self.auxiliary_branch)
        if freeze_bn:
    ...
    def get_backbone_params(self):
        return chain(self.block0.parameters(), self.block1.parameters(),
            self.block2.parameters(), self.block3.parameters(), self.
            transition1.parameters(), self.transition2.parameters(), self.
            transition3.parameters())
