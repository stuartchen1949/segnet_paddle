    ...
    def __init__(self, in_channels=3, output_stride=16, backbone=\
        ...
        if not pretrained or in_channels != 3:
            self.layer0 = nn.Sequential(nn.Conv2D(in_channels, 64, 7,
                stride=2, padding=3, bias_attr=False), nn.BatchNorm2D(64),
                x2paddle.torch2paddle.ReLU(inplace=True), nn.MaxPool2D(
    ...
    def __init__(self, in_channels, out_channels, stride=1, dilation=1,
        ...
        if in_channels != out_channels or stride != 1:
            self.skip = nn.Conv2D(in_channels, out_channels, 1, stride=\
                stride, bias_attr=False)
            self.skipbn = nn.BatchNorm2D(out_channels)
        else:
            self.skip = None
        rep = []
        self.relu = x2paddle.torch2paddle.ReLU(inplace=True)
...
class Xception(nn.Layer):
    ...
    def __init__(self, output_stride=16, in_channels=3, pretrained=True):
        ...
        self.conv1 = nn.Conv2D(in_channels, 32, 3, 2, padding=1, bias_attr=\
            False)
        self.bn1 = nn.BatchNorm2D(32)
        self.relu = x2paddle.torch2paddle.ReLU(inplace=True)
        self.conv2 = nn.Conv2D(32, 64, 3, 1, padding=1, bias_attr=False)
    ...
    def _load_pretrained_model(self):
        url = (
            'http://data.lip6.fr/cadene/pretrainedmodels/xception-b5690688.pdiparams'
            )
        pretrained_weights = model_zoo.load(url)
...
def assp_branch(in_channels, out_channles, kernel_size, dilation):
    ...
    return nn.Sequential(nn.Conv2D(in_channels, out_channles, kernel_size,
        padding=padding, dilation=dilation, bias_attr=False), nn.
        BatchNorm2D(out_channles), x2paddle.torch2paddle.ReLU(inplace=True))
...
class ASSP(nn.Layer):
    ...
    def __init__(self, in_channels, output_stride):
        ...
        self.avg_pool = nn.Sequential(nn.AdaptiveAvgPool2D((1, 1)), nn.
            Conv2D(in_channels, 256, 1, bias_attr=False), nn.BatchNorm2D(
            256), x2paddle.torch2paddle.ReLU(inplace=True))
        self.conv1 = nn.Conv2D(256 * 5, 256, 1, bias_attr=False)
    ...
    def forward(self, x):
        ...
        x = self.conv1(torch2paddle.concat((x1, x2, x3, x4, x5), dim=1))
        x = self.bn1(x)
...
class Decoder(nn.Layer):
    ...
    def __init__(self, low_level_channels, num_classes):
        ...
        self.conv1 = nn.Conv2D(low_level_channels, 48, 1, bias_attr=False)
        self.bn1 = nn.BatchNorm2D(48)
        self.relu = x2paddle.torch2paddle.ReLU(inplace=True)
        self.output = nn.Sequential(nn.Conv2D(48 + 256, 256, 3, stride=1,
            padding=1, bias_attr=False), nn.BatchNorm2D(256), x2paddle.
            torch2paddle.ReLU(inplace=True), nn.Conv2D(256, 256, 3, stride=\
            1, padding=1, bias_attr=False), nn.BatchNorm2D(256), x2paddle.
            torch2paddle.ReLU(inplace=True), nn.Dropout(0.1), nn.Conv2D(256,
            num_classes, 1, stride=1))
        initialize_weights(self)
    ...
    def forward(self, x, low_level_features):
        ...
        x = self.output(torch2paddle.concat((low_level_features, x), dim=1))
        return x
...
class DeepLab(BaseModel):
    ...
    def __init__(self, num_classes, in_channels=3, backbone='xception',
        pretrained=True, output_stride=16, freeze_bn=False, freeze_backbone
        =False, **_):
        super(DeepLab, self).__init__()
        assert 'xception' or 'resnet' in backbone
        if 'resnet' in backbone:
            self.backbone = ResNet(in_channels=in_channels, output_stride=\
                output_stride, pretrained=pretrained)
