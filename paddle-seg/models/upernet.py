...
class PSPModule(nn.Layer):
    ...
    def __init__(self, in_channels, bin_sizes=[1, 2, 4, 6]):
        ...
        self.bottleneck = nn.Sequential(nn.Conv2D(in_channels + 
            ...
            padding=1, bias_attr=False), nn.BatchNorm2D(in_channels),
            x2paddle.torch2paddle.ReLU(inplace=True), nn.Dropout(0.1))
...
class FPN_fuse(nn.Layer):

    def __init__(self, feature_channels=[256, 512, 1024, 2048], fpn_out=256):
        super(FPN_fuse, self).__init__()
        assert feature_channels[0] == fpn_out
        self.conv1x1 = nn.LayerList([nn.Conv2D(ft_size, fpn_out,
            kernel_size=1) for ft_size in feature_channels[1:]])
        self.smooth_conv = nn.LayerList([nn.Conv2D(fpn_out, fpn_out,
            kernel_size=3, padding=1)] * (len(feature_channels) - 1))
        self.conv_fusion = nn.Sequential(nn.Conv2D(len(feature_channels) *
            fpn_out, fpn_out, kernel_size=3, padding=1, bias_attr=False),
            nn.BatchNorm2D(fpn_out), x2paddle.torch2paddle.ReLU(inplace=True))
    ...
    def forward(self, features):
        ...
        x = self.conv_fusion(torch2paddle.concat(P, dim=1))
        return x


class UperNet(BaseModel):

    def __init__(self, num_classes, in_channels=3, backbone='resnet101',
        pretrained=True, use_aux=True, fpn_out=256, freeze_bn=False, **_):
    ...
    def get_decoder_params(self):
        return chain(self.PPN.parameters(), self.FPN.parameters(), self.
            head.parameters())
